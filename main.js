...
class App extends React.Component {
  ...
  componentDidMount() {
    if (this.state.activePage === "right") {
      ...
      var _this = this;
      var url = "https://josephboman.com/json/" + arr[arr.length - 1] + ".json";
      $.ajax({
        dataType: "json",
        url: url,
  ...
  componentDidUpdate() {
    ...
    if (this.state.activePage === "right") {
      ...
      $(".active").animate({ left: "0%" }, 990, function () {
        if (
          typeof _this.state.activeLink !== "undefined" &&
          _this.state.activeLink !== null
        )
  ...
  handleNavigation(link, moveTo, activeLink) {
    ...
    let rightBuffer = null,
      bufferType = null;
    var url = "https://josephboman.com/json/" + link + ".json";
    var _this = this;
    ...
    if (moveTo === "right") {
      ...
      this.setState({
        ...
        activeLink: activeLink,
        rightBuffer:
          rightBuffer === null ? this.state.rightBuffer : rightBuffer,
  ...
  handleMobileMenu(link, activePage, mobileLink) {
    ...
    if (activePage === "right") {
      ...
      if (link !== this.state.currentPage) {
        ...
        if (
          typeof this.state.activeLink !== "undefined" &&
      ...
      this.handleNavigation(
        link,
        activePage,
        typeof this.state[link] != "undefined" ? this.state[link] : null
      );
    } else {
      NProgress.start();
      history.pushState(
        { page: link },
        link,
        "https://josephboman.com/" + link
      );
      $(".main-page").css({ display: "block" });
      var scrollTarget = $(link).offset().top - 80;
      if (link === "#contact") scrollTarget = document.body.scrollHeight + 200;
      if (this.state.activePage === "right") {
        this.state.scrollTarget = scrollTarget;
  ...
  handleBack(event) {
    ...
    if (event.state.page == "main" || event.state.page.indexOf("#") != -1) {
      ...
      let rightBuffer = null,
        bufferType = null;
      var _this = this;
      var url = "https://josephboman.com/json/" + event.state.page + ".json";
      $.ajax({
        dataType: "json",
        url: url,
        success: function (data) {
...
function FrontPage(props) {
  return (
    <div>
      <PageBody
        topSection={<FrontPageTop />}
        bottomSection={
          <div>
            <About handleClick={props.mobileMenu} />
            <Projects
  ...
  return (
    <header id="header">
      <nav className="nav-bar">
        <a href="/">
          <img src="https://josephboman.com/images/logo.png" />
          <p>
            <span>oseph</span> oman
          </p>
        </a>
  ...
  return (
    <footer id="footer">
      <div className="footer">
        ...
        <div>
          <a className="text-link email" href="mailto:joseph.j.boman@gmail.com">
...
function PageBody(props) {
  return (
    <div className="page">
      <div className="page-top">{props.topSection}</div>
      <div className="page-bottom">{props.bottomSection}</div>
    </div>
  );
}
  ...
  render() {
    return (
      <a
        href={this.props.link}
        className={
          "text-link" + (this.props.sublink === "true" ? " sub-link" : "")
        }
        onClick={this.handleClick}
      >
        {this.props.name}
  ...
  render() {
    return (
      <div id="about">
        <h3>ABOUT ME</h3>
        <p>
          Programming and Games have long been two of my greatest passions, and
          my skill at them has only increased my interest. Both allow me to be
          both logical and creative, to solve tricky design and programming
          problems, and to explore the limits of imagination.
          <br />
          <br />I graduated from USC, where I studied Computer Science (Games),
          in 2016, and have worked at Kaiser Permanente since then. While at
          USC, I got the chance to work on several board games and video games,
          including one that shipped on PS4. I also worked on a couple of
          research projects, which you can see more about by looking at the
          project pages or my{" "}
          <a class="text-link" href="resume" onClick={this.toResume}>
            Resume
          </a>
          .<br />
          <br />
          When I'm not immersed in some form of fictional media or outdoors, I
          enjoy playing piano, spending time with friends, and learning about
          whatever topic piques my interest. Exploring created worlds is a great
          passion of mine, and Earth is no exception; if I have a chance to
          spend time in nature, I will seize the opportunity. If you are
          interested in working with me, feel free to{" "}
          <a class="text-link" href="#contact" onClick={this.toContact}>
            contact me
          </a>{" "}
  ...
  cycleImage(slideshow) {
    if (slideshow.state.cycleImage) {
      slideshow.switchImage(
        (slideshow.state.activeImage + 1) % slideshow.props.images.length,
        false
      );
    }
  }
      ...
      <div className="img-slideshow">
        <div className="circle-img" onClick={this.handleClick}>
          {images.map((image, index) => (
            <img key={image.link} src={image.link} alt={image.alt} />
          ))}
        </div>
        <div className="img-selector">
          {images.map((image, index) => (
            <Dot
              key={image.link + "-dot"}
              number={index}
              active={index === this.state.activeImage ? true : false}
              handleClick={this.switchImage}
            />
          ))}
        </div>
      </div>
          ...
          <div className="right-buffer" type={this.props.type}>
            ...
            <div className="modal" onClick={this.closeModal}>
              <div className="modal-content">
                ...
                <img
                  src={this.props.data.images[this.state.activeImage].link}
              ...
              topSection={
                <div>
                  ...
                  >
                    ...
                    {this.props.data.tags.map((detail) => (
                      <TagDetail
                        key={detail.name}
                        name={detail.name}
                        text={detail.text}
              ...
              topSection={
                <div>
                  <BackArrow
                    ...
                    handleBack={this.props.handleBack}
                  />
                  <h1 style={{ marginTop: "10px", paddingTop: "20px" }}>
              ...
              bottomSection={
                <div>
                  ...
                  <h2>
                    ...
                    <br />
                    <br />
                    Please navigate to another page via the header, back
                    buttons, or another URL
                  </h2>
                  <BackArrow
                    height="40"
                    width="40"
  ...
  return (
    <p>
      {arr.map(function (stringPart, index) {
        if (index % 2 === 0) return stringPart;
        else {
          var arr = stringPart.split("--");
          return (
            <a key={arr[0] + arr[1]} href={arr[0]} alt={arr[1]} target="_blank">
              {arr[1]}
            </a>
          );
        }
  ...
  return (
    <div>
      ...
      {props.data.map((section) => (
        <div key={section.name + section.subname}>
          <h4>
            {section.name}
            <span className="right time">{section.time}</span>
            <span className="right longTime">{section.longTime}</span>
          </h4>
          <h5>{section.subname}</h5>
          <p>{section.details}</p>
        </div>
      ))}
    </div>
  ...
  render() {
    return (
      <a
        ...
        className={"block-link project" + (this.state.active ? " active" : "")}
        onClick={this.handleClick}
      >
        <Tags tags={this.props.tags} />
        <h4
          className={
            this.props.name.length >= 10 ? "thin-margin" : "thick-margin"
          }
        >
          {this.props.name}
        </h4>
